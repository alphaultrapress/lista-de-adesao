...
/* --------------------------------------------------------------------------
   Seções institucionais da landing, extraídas para serem reaproveitadas.

   A página inicial e a página de adesão (o link que o representante manda para
   a turma) mostram exatamente os mesmos blocos. Mantê-los aqui evita que as
   duas versões comecem a divergir a cada ajuste de texto.
   -------------------------------------------------------------------------- */
...
/** Faixa preta com os números da Alpha.
 *  `sobCarrossel` reserva o espaço que as peças do carrossel do herói invadem —
 *  só a landing precisa disso. */
export function FaixaNumeros({ sobCarrossel = false }: { sobCarrossel?: boolean }) {
  return (
    <>
      {/* ══ FAIXA PRETA ══ números como elemento dominante */}
      <section
        className={`bg-obsidian pb-20 md:pb-[120px] ${sobCarrossel ? "" : "pt-20 md:pt-[120px]"}`}
        ...
        style={{
          paddingTop: sobCarrossel
            ? `calc(var(--hero-card) * ${1 - SOBRE_VIDEO} + 72px)`
            : undefined,
        }}
      >
...
/** "Como funciona" — os quatro passos, com alternador celular/computador. */
...
/** Cabeçalho em duas colunas + carrossel dos sete acabamentos. */
...
/** Herói da landing: vídeo full-bleed + carrossel de peças.
 *  `children` entra entre o título e o botão — é onde a página de adesão
 *  identifica a turma. */
...
/** Bloco de fechamento com os pontos Vanta ao fundo. O `children` ocupa o
 *  lugar do botão: na landing é o CTA, na adesão é o formulário. */
...
/** Rodapé. `comConta` esconde Cadastrar/Entrar em páginas que não criam conta. */
